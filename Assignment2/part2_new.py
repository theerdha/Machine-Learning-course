...
def in_sample(W1,W2,W3):
	...
	for instance2 in training:
		...
		if h != instance2[-1] :
			misclass = misclass + 1
		test_error += 0.5 * ((h - instance2[-1]) ** 2)
...
while epochs > 0:
	for instance in training:
		...
		# 	break
		dell = (h - instance[-1]) * sigmoid_derivative(h)

		del3 = []
		if h == 0:
			del3.append(softmax[0][0] * (1 - softmax[0][0]))
			del3.append(-softmax[0][0] * softmax[0][1])
		else:
			del3.append(-softmax[0][1] * (softmax[0][0]))
			del3.append(softmax[0][1] * (1 - softmax[0][1]))
		del3 = np.asarray(del3)
		del3 = del3 * dell
